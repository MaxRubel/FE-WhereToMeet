...
export default function SingleEventLayout() {
  ...
  return (
    <div className="profile-page-layout" style={{ textAlign: "left" }}>
        ...
        <ul className="profile-list">
          ...
          {user._id === event.ownerId && (
            <button
              className="clear-button side-list-item"
              onClick={() => {
                setIsViewing("moderatorSettings");
              }}
              style={{
                backgroundColor:
      ...
      {isViewing === "singleEvent" && <SingleEventComponent event={event} />}

      {isViewing === "editEvent" && (
        <CreateEventForm event={event} setIsViewing={setIsViewing} />
      )}
      {isViewing === "moderatorSettings" && (
        <ModeratorSettings event={event} setIsViewing={setIsViewing} />
      )}
    </div>
  );
}
